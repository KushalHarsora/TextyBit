  ...
  return (
    <main className=' h-screen w-screen flex flex-col justify-center items-center z-0 overflow-x-hidden'>
      ...
      <section className=' fixed top-0 left-0 h-[9vh] w-screen flex flex-row justify-center items-center overflow-hidden bg-transparent backdrop-blur-md z-20 border-b-[0.25px] border-gray-100'>
        <div className=' h-fit w-[20vw] overflow-hidden flex justify-center items-center max-lg:w-1/2'>
          <Image src={'logo.svg'} alt='logo' width={120} height={120} priority/>
        </div>
        <div className=' h-full w-[50vw] flex flex-row justify-center items-center max-lg:hidden'>
          <Button variant={'link'}>Pricing</Button>
          <Button variant={'link'}>Affiliate</Button>
          <Button variant={'link'}>Careers</Button>
        </div>
        <div className=' h-full w-[30vw] flex flex-row justify-center items-center gap-4 max-lg:w-1/2'>
      ...
      <section className=' h-fit w-screen relative top-[9vh] left-0 flex flex-col justify-center items-center'>
        <div className=' flex flex-col justify-center items-center gap-8 w-4/5 max-lg:w-full'>
          <button className=' bg-white text-black rounded-md shadow-md p-2 px-3 font-semibold text-sm cursor-default max-lg:text-xs'>
            TextyBit is now Public
          </button>
          <div className=' flex flex-col justify-center items-center gap-0'>
          <h1 className=' text-7xl font-semibold max-lg:text-2xl'>
            Chat with your
          </h1>
          <h1 className=' text-7xl font-semibold flex flex-row max-lg:text-2xl'>
            <FlipText word='Documents' className=' font-bold text-blue-700'></FlipText>
            <span>&nbsp;in seconds.</span>
          </h1>
          </div>
          <Button className=' bg-white text-black rounded-md shadow-md shadow-gray-300 hover:text-white hover:bg-black hover:font-medium max-lg:text-white max-lg:bg-black max-lg:text-xs max-lg:hover:text-white max-lg:hover:bg-black' onClick={() => router.push("/register")}>
            Getting Started
